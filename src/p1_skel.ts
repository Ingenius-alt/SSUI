...
class FittsTestUI extends UIClass {
    ...
    override configure(newState : string) {
        ...
        switch (this.currentState) {
            // the start state will hide all elements except the background which will give
            // directions to start the Fitts tester
            case 'start':
            ...
            break;
            // the begin_trial state will show what current trial we are on while
            // hiding the target and revealing the reticle
            case 'begin_trial':
                this.theBackground.msg1 = "Trial #"+this.trialCount;
                this.theBackground.msg2 = "";
                this.theBackground.msg3 = "";
            ...
            break;
            // the in_trial state will show the target and nothing else
            case 'in_trial':
                this.theReticle.visible = false;
            ...
            break;
            // Hides everything except the message that it is done
            case 'ended':
    ...
    newTrial() {
        ...
        if (this.trialCount > this.MAX_TRIALS) {
            ...
            const {retX:retX, retY:retY, targX:targX, targY:targY, targD:targDiam} = 
                pickLocationsAndSize(this.canvas.width,this.canvas.height);
            this.theReticle.x = retX;
            this.theReticle.y = retY;
...
class Target extends ScreenObject{
    ...
    newGeom(newCentX : number, newCentY : number, newDiam? : number) {
        // Changes the position of the target as well as 
        // the diameter and radius then redraws
        this.x = newCentX;
        this.y = newCentY;
    ...
    override draw(ctx : CanvasRenderingContext2D) : void {
        ...
        ctx.beginPath();
        ctx.arc(this._x,this.y,this.radius,0,2*Math.PI,false);
        ctx.fill();
        ctx.stroke();
    ...
    override pickedBy(ptX : number, ptY : number) : boolean {
        ...
        const distance : number = Math.sqrt((ptX - this.x) ** 2 + (ptY - this.y) ** 2);
...
class Reticle extends Target {
    ...
    override draw(ctx : CanvasRenderingContext2D) : void {
        // This draws the inner and outer circle as
        // well as the sights if visible
        ...
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(this.x,this.y,this.w/10,0,2*Math.PI,false);
        ctx.fill();
        ctx.stroke();
        ctx.beginPath();
    ...
    override pickedBy(ptX : number, ptY : number) : boolean {
        // calculates the distance of where was clicked to respect
        // to the reticles position if it is greater than the radius
        // of the inner circle return false otherwise true
        const distance : number = Math.sqrt((ptX - this.x) ** 2 + (ptY - this.y) ** 2);
        return distance < Reticle.RETICLE_INNER_DIAM;
    }
    ...
    override handleClickAt(ptX : number, ptY : number) : boolean {
        // if we are at the beginning and it was clicked correctly
        // then we change state as well as start a new trial
        if(this._parentUI.currentState === "begin_trial" && this.pickedBy(ptX,ptY)) {
...
class BackgroundDisplay extends ScreenObject{
    ...
    override handleClickAt(ptX : number, ptY : number) : boolean {
        // if we are at the start state then just start a new trial
        // by changing the state and starting the trail
